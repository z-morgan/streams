...
Read plan.md and create one beads issue per logical step, using the metadata field to encode execution order:
  bd create --parent %s --title="<descriptive action>" --type=task --priority=2 --metadata '{"sequence":N}' --description="<what to do in this step>"

Example:
  bd create --parent %s --title="Scaffold Go module and directory layout" --type=task --priority=2 --metadata '{"sequence":1}' --description="..."
  bd create --parent %s --title="Define runtime interface" --type=task --priority=2 --metadata '{"sequence":2}' --description="..."
...
2. For each open child issue that is feedback (not a step bead), read it, adjust the step beads accordingly (create, update metadata, or close), and close the feedback issue with bd close.
