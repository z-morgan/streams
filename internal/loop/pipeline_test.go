...
func TestFullPipelineGateResults(t *testing.T) {
	s := newTestStream()
	s.Pipeline = []string{"coding"}
	s.PipelineIndex = 0

	// Review text contains gate-triggering keywords
	rt := &mockRuntime{
		results: []mockResult{
			{resp: &runtime.Response{Text: "implemented the feature"}},
			{resp: &runtime.Response{Text: "this is unnecessarily complex and the pattern is inconsistent"}},
		},
	}
	...
	snap := s.Snapshots[0]
	if len(snap.GateResults) != 4 {
		t.Fatalf("expected 4 gate results, got %d", len(snap.GateResults))
	}

	failedCount := 0
	for _, gr := range snap.GateResults {
		if !gr.Passed {
			failedCount++
		}
	}
	if failedCount == 0 {
		t.Error("expected at least some gates to fail")
	}
}
